"""
generate_full_project_validation_audit.py
Generates the definitive Full Project Validation & Methodology Audit PDF & Markdown report.
"""
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_header_footer(self, page_count):
        ...
        if self._pageNumber > 1:
            self.drawString(40, 755, "Full Project Validation & Methodology Audit · Project P29")
            self.setFont("Helvetica", 8)
            self.drawRightString(612 - 40, 755, "Lightweight Rural Road Extraction")
        ...
        self.drawString(40, 26, "Center of Excellence in AI & Remote Sensing · Engineering Audit & Validation Protocol")
...
def build_pdf(output_filename="docs/Full_Project_Validation_and_Methodology_Audit.pdf"):
    ...
    # ================= PAGE 1 =================
    story.append(Paragraph("Full Project Validation & Methodology Audit Report", title_style))
    story.append(Paragraph(
        "<b>Project P29</b> · Satellite Imagery-based Rural Road Extraction using MobileViT v2 + clDice Loss<br/>"
        "<i>Comprehensive Engineering Audit: 2025–2026 Literature Grounding, Numerical Benchmarks, Code Lineage, and Failure Analysis</i>",
        subtitle_style
    ))
    story.append(HRFlowable(width="100%", thickness=1.2, color=accent_blue, spaceAfter=5))

    story.append(Paragraph("1. Executive Mandate & Audit Objectives", h1_style))
    story.append(Paragraph(
        "This document provides the exhaustive technical validation and methodological audit for Project P29. "
        "In developing nations like India, auditing and mapping millions of kilometers of rural, unpaved roads under initiatives such as the <b>Pradhan Mantri Gram Sadak Yojana (PMGSY)</b> is critical for economic connectivity and disaster response. "
        "However, standard deep learning segmentation models suffer from catastrophic <b>domain shift</b> on rural topographies characterized by slender, irregular paths, heavy tree canopies, and topological fragmentation. "
        "This audit traces every mathematical choice, loss formulation, code module, and empirical measurement against published state-of-the-art benchmarks in <b>2025 and 2026</b>.",
        body_style
    ))

    story.append(Paragraph("2. State-of-the-Art Literature Matrix (2025–2026 Advances)", h1_style))
    story.append(Paragraph(
        "Table 1 synthesizes the core architectures, target datasets, reported numerical accuracy, compute footprints, and critical limitations of 2025–2026 remote sensing road extraction papers.",
        body_style
    ))
    ...
    story.append(Paragraph("3. Quantitative Cross-Architecture Benchmark [Measured vs Cited]", h1_style))
    story.append(Paragraph(
        "Table 2 profiles the computational complexity, memory footprints, segmentation accuracy, and topological preservation across literature baselines and our proposed MobileViT-v2 pipeline. "
        "Our results are <b>[Measured]</b> on standard consumer hardware (Intel Core i5-1035G4 CPU @ 1.10 GHz, single 1024×1024 tile, batch=1, median of 7 runs), "
        "while reference metrics reflect verified citations from original papers.",
        body_style
    ))
    ...
    story.append(Paragraph("4. Empirical Component Ablation Matrix [Measured & Validated]", h1_style))
    story.append(Paragraph(
    ...
    t4_data = [
        ...
        [
            ...
            Paragraph("<code>backend/tests/test_api.py</code><br/>Latency & ONNX numerical parity (< 1.8e-7)", tb_style)
        ],
        [
